...
router.post('/', canWrite, async (req, res) => {
  ...
  const { data, error } = await supabase
    .from('tasks')
    .insert({ title, description, status, priority, assignee, tags, due_date: dueDate, created_by: req.user.name })
    .select().single();
...
router.put('/:id', canWrite, async (req, res) => {
  ...
  await supabase.from('tasks').update(update).eq('id', req.params.id);
  res.json({ ok: true });
});

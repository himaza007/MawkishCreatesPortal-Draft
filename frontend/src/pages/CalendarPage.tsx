  ...
  return (
    <>
      <Topbar title="Calendar">
        {editor && <Button onClick={() => setModal(true)} size="sm"><Plus size={14} /> Add Event</Button>}
      </Topbar>
      <Page>
        <Grid2>
          <Card>
          ...
          <Card>
            ...
            {sorted.map(e => (
              <div key={e._id} className={styles.eventRow}>
                ...
                {editor && <button className={styles.delBtn} onClick={() => remove(e._id)}><Trash2 size={13} /></button>}
              </div>
            ))}
          </Card>
        </Grid2>
      ...
      {editor && <Modal open={modal} onClose={() => setModal(false)} title="New Event"
